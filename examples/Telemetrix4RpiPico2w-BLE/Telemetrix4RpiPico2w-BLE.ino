...
void servo_attach() {
  ...
  if (servo_found != -1) {
    ...
    BLE_Uart.write(report_message, 2);
  }
}
...
void i2c_read() {
  ...
  if (num_of_bytes < current_i2c_port->available()) {
    ...
    BLE_Uart.write(report_message, 4);
    return;
  ...
  for (int i = 0; i < message_size + 6; i++) {
    BLE_Uart.write(i2c_report_message[i]);
  }
}
...
void read_blocking_spi() {
  ...
  BLE_Uart.write(spi_report_message, number_of_bytes + 6);
}
...
void onewire_reset() {
  ...
  //BLE_Uart.write(onewire_report_message, 4);
}
...
void onewire_search() {
  ...
  //BLE_Uart.write(onewire_report_message, 11);
}
...
void stepper_get_distance_to_go() {
  ...
  // motor_id = command_buffer[0]
  BLE_Uart.write(report_message, 7);
}
...
void get_next_command() {
  ...
  if (not BLE_Uart.available()) {
    return;
  }
  // get the packet length
  //Serial.println("Available");
  packet_length = (byte)BLE_Uart.read();
  //Serial.println(packet_length);

  while (not BLE_Uart.available()) {
    delay(1);
  }

  // get the command byte
  command = (byte)BLE_Uart.read();
  ...
  if (packet_length > 1) {
    ...
    for (int i = 0; i < packet_length - 1; i++) {
      ...
      command_buffer[i] = (byte)BLE_Uart.read();
  ...
  if (monitor_cpu_temp) {
    ...
    if (cpu_temp_current_millis - cpu_temp_previous_millis > cpu_temp_sampling_interval) {
      ...
      if (differential >= cpu_temp_threshold) {
        ...
        BLE_Uart.write(report_message, 6);
      }
    }
  }
}
  ...
  if (dht_index) {
    ...
    if (dht_current_millis - dht_previous_millis > dht_scan_interval) {
      ...
      for (int i = 0; i < dht_index; i++) {
        ...
        if (rv) {
          BLE_Uart.write(report_message, 10);
          return;
        } else {
          float j, f;
...
void get_pico_unique_id() {
  ...
  BLE_Uart.write(unique_id_report_report_message, 10);
}
      ...
      switch (stepper_run_modes[i]) {
        case STEPPER_RUN:
          ...
          if (!running) {
            ...
            BLE_Uart.write(report_message, 3);
...
void setup() {
  ...
  BLE.server()->addService(&BLE_Uart);
